...
named!(pub parse_message<&[u8], Message>,
    do_parse!(
        ...
        dhcp_message_type           : opt!(preceded!(tag!(&[DhcpMessageType as u8, U8_LEN])     , u8_to_message_type)) >>
        dhcp_server_id              : opt!(preceded!(tag!(&[DhcpServerId as u8, U32_LEN])       , u32_to_ipv4)) >>
...
named!(u8_to_message_type<&[u8], MessageType>,
    map!(be_u8, |value| value.into())
);

named!(u32_to_ipv4<&[u8], Ipv4Addr>,
    map!(be_u32, |value| Ipv4Addr::from(value))
);

named!(bytes_to_vec<&[u8], Vec<u8>>,
    map!(length_bytes!(be_u8), |value| value.to_vec())
);

named!(bytes_to_string<&[u8], String>,
    map!(length_bytes!(be_u8), |value| String::from_utf8_lossy(value).to_string())
);

const IPV4_LEN: usize = 4;
named!(bytes_to_ipv4_vec<&[u8], Vec<Ipv4Addr>>,
...
named!(bytes_to_ipv4_pairs_vec<&[u8], Vec<(Ipv4Addr, Ipv4Addr)>>,
    map!(length_bytes!(be_u8), |value| {
