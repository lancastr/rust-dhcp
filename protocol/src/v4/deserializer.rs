...
impl Message {
    ...
    pub fn from_bytes(src: &[u8]) -> io::Result<Self> {
        ...
        let mut message = Message {
            ...
            server_name: {
                let string =
                    String::from_utf8_lossy(&src[distance!(cursor, SIZE_SERVER_NAME)]).into();
                cursor.advance(SIZE_SERVER_NAME);
                string
            },
            boot_filename: {
                let string =
                    String::from_utf8_lossy(&src[distance!(cursor, SIZE_BOOT_FILENAME)]).into();
        ...
        match message.options.overload {
            Some(Overload::File) => {
                let mut cursor =
                    ::std::io::Cursor::new(&src[OFFSET_BOOT_FILENAME..SIZE_BOOT_FILENAME]);
                Self::append_options(&mut cursor, &mut message.options)?;
            }
            Some(Overload::Sname) => {
    ...
    fn append_options(mut cursor: &mut io::Cursor<&[u8]>, options: &mut Options) -> io::Result<()> {
        while cursor.remaining() > 0 {
            ...
            match tag.into() {
                ...
                DomainNameServers => {
                    options.domain_name_servers = Some(Self::get_vec_ipv4(&mut cursor)?)
                }
                ...
                NonLocalSourceRouteOnOff => {
                    options.non_local_source_route_on_off = Some(Self::get_u8(&mut cursor)?)
                }
                PolicyFilters => {
                    options.policy_filters = Some(Self::get_vec_ipv4_pairs(&mut cursor)?)
                }
                MaxDatagramReassemblySize => {
                    options.max_datagram_reassembly_size = Some(Self::get_u16(&mut cursor)?)
                }
                ...
                PerformRouterDiscovery => {
                    options.perform_router_discovery = Some(Self::get_u8(&mut cursor)?)
                }
                RouterSolicitationAddress => {
                    options.router_solicitation_address = Some(Self::get_ipv4(&mut cursor)?)
                }
                StaticRoutes => {
                    options.static_routes = Some(Self::get_vec_ipv4_pairs(&mut cursor)?)
                }
                TrailerEncapsulation => {
                    options.trailer_encapsulation = Some(Self::get_u8(&mut cursor)?)
                }
                ArpTimeout => options.arp_timeout = Some(Self::get_u32(&mut cursor)?),
                EthernetEncapsulation => {
                    options.ethernet_encapsulation = Some(Self::get_u8(&mut cursor)?)
                }
                ...
                NetbiosNameServers => {
                    options.netbios_name_servers = Some(Self::get_vec_ipv4(&mut cursor)?)
                }
                NetbiosDistributionServers => {
                    options.netbios_distribution_servers = Some(Self::get_vec_ipv4(&mut cursor)?)
                }
                ...
                XWindowFontServers => {
                    options.x_window_font_servers = Some(Self::get_vec_ipv4(&mut cursor)?)
                }
                XWindowManagerServers => {
                    options.x_window_manager_servers = Some(Self::get_vec_ipv4(&mut cursor)?)
                }
                ...
                DhcpMessageType => {
                    options.dhcp_message_type = Some(Self::get_u8(&mut cursor)?.into())
                }
                ...
                DhcpMaxMessageSize => {
                    options.dhcp_max_message_size = Some(Self::get_u16(&mut cursor)?)
                }
                ...
                HomeAgentAddresses => {
                    options.home_agent_addresses = Some(Self::get_vec_ipv4(&mut cursor)?)
                }
                ...
                StreetTalkServers => {
                    options.street_talk_servers = Some(Self::get_vec_ipv4(&mut cursor)?)
                }
